@php
  $isPublicResourceDirectory = is_tax('product', 'public-resource-directory');
  $sectionClass = $isPublicResourceDirectory ? 'guides-public-resource-directory-header' : '';
@endphp

<style>

/* Ensure "Public Resource Center Help Center" is always visible */
.guides-public-resource-directory-header {
  display: block !important;
}

</style>

<section class="component-section mb-12 sm:mb-16 {{ $sectionClass }}">
    <div class="component-inner-section relative">
        <div
            class="relative flex flex-col items-center sm:grid grid-cols-12 gap-10 text-white bg-brand p-4 sm:p-12 rounded-2xl overflow-hidden">
            <div class="absolute inset-0 z-0"
                style="background: url(@asset('/images/header-bg.png')) center center; background-size: cover;">
            </div>
                ...
                @isset($product['image'])
                    <div>
                        <img class="mx-auto w-full h-auto max-w-[194px] xl:max-w-[225px]"
                            src="{{ $product['image']['sizes']['medium_large'] }}" alt="" />
                    </div>
                    @endif
                </div>
            </div>
